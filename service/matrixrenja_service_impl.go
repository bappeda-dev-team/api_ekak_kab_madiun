...
func (service *MatrixRenjaServiceImpl) GetRenjaPenetapan(ctx context.Context, kodeOpd, tahun, jenisPagu string) ([]programkegiatan.UrusanDetailResponse, error) {
	...
	result := service.transformToResponsePenetapan(data, kodeOpd, tahun)
	if err := tx.Commit(); err != nil {
		return nil, err
	}
	return result, nil
...
func (service *MatrixRenjaServiceImpl) transformToResponsePenetapan(data []domain.SubKegiatanQuery, kodeOpd, tahun string) []programkegiatan.UrusanDetailResponse {
	if len(data) == 0 {
		return []programkegiatan.UrusanDetailResponse{}
	}
	...
	for _, item := range data {
		...
		if _, ok := seenSubkeg[item.KodeSubKegiatan]; !ok {
			...
			if item.JenisPagu == "penetapan" {
				paguSubkeg[item.KodeSubKegiatan] =
					item.PaguSubKegiatan + item.TotalAnggaranSubKegiatan
			} else {
				paguSubkeg[item.KodeSubKegiatan] = 0
			}
