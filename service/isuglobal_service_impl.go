...
func (service *IsuGlobalServiceImpl) FindByIds(ctx context.Context, request isuglobal.FindByIdsRequest) ([]isuglobal.IsuGlobalResponse, error) {
	...
	results, err := service.IsuGlobalRepository.FindByIds(ctx, tx, request.Ids)
	if err != nil {
		return nil, err
	}

	responses := make([]isuglobal.IsuGlobalResponse, 0, len(results))

	for _, result := range results {
		responses = append(responses, isuglobal.IsuGlobalResponse{
			ID:               result.ID,
			KodeBidangUrusan: result.KodeBidangUrusan,
			KodeOpd:          result.KodeOpd,
			Isu:          result.Isu,
