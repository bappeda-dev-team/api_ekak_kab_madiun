...
func (service *IsuRegionalServiceImpl) FindByIds(ctx context.Context, request isuregional.FindByIdsRequest) ([]isuregional.IsuRegionalResponse, error) {
	...
	results, err := service.IsuRegionalRepository.FindByIds(ctx, tx, request.Ids)
	if err != nil {
		return nil, err
	}

	responses := make([]isuregional.IsuRegionalResponse, 0, len(results))

	for _, result := range results {
		responses = append(responses, isuregional.IsuRegionalResponse{
			ID:               result.ID,
			KodeBidangUrusan: result.KodeBidangUrusan,
			KodeOpd:          result.KodeOpd,
			Isu:          result.Isu,
