...
func (c *IsustrategicClientImpl) GetDataIsuStrategic(ctx context.Context, kodeOpd string, tahun string) ([]IsuStrategisResponse, error) {
...
func (c *IsustrategicClientImpl) GetDataPermasalahan(ctx context.Context, kodeOpd string, tahun string) ([]PermasalahanResp, error) {
	url := fmt.Sprintf("%s/isu_strategis/kebelakang/%s/%s", c.host, kodeOpd, tahun)
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, url, nil)
	if err != nil {
		return nil, err
