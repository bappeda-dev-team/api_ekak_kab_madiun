...
func (repository *PohonKinerjaRepositoryImpl) FindById(ctx context.Context, tx *sql.Tx, id int) (domain.PohonKinerja, error) {
	scriptPokin := `
        SELECT 
            ...
            COALESCE(opd.nama_opd, '') as nama_opd,
        ...
        FROM 
            tb_pohon_kinerja pk
	LEFT JOIN tb_operasional_daerah opd ON opd.kode_opd = pk.kode_opd
        WHERE 
            pk.id = ?`
...
// pokin opd yang pohon parentnya ada
func (repository *PohonKinerjaRepositoryImpl) FindAllPokinOpdForCetak(ctx context.Context, tx *sql.Tx, kodeOpd string, tahun int) ([]domain.PohonKinerja, error) {
	script := `
		WITH RECURSIVE pohon AS (
		SELECT id, parent, nama_pohon, tahun, level_pohon, jenis_pohon
		FROM tb_pohon_kinerja
