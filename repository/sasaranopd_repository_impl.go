...
func (repository *SasaranOpdRepositoryImpl) FindByNipAndOpd(
    ...
    WHERE pk.level_pohon = 4
      ...
      AND CAST(pk.tahun AS SIGNED) >= CAST(so.tahun_awal AS SIGNED)
      AND CAST(pk.tahun AS SIGNED) <= CAST(so.tahun_akhir AS SIGNED)
      AND CAST(? AS SIGNED) BETWEEN CAST(so.tahun_awal AS SIGNED) AND CAST(so.tahun_akhir AS SIGNED)
    ORDER BY pk.nama_pohon ASC, so.nama_sasaran_opd ASC`
	...
	for rows.Next() {
		...
		so, exists := pokinMap[pokinId]
		if !exists {
			so = &domain.SasaranOpd{
		...
		if sasaranId.Valid {
			...
			for i := range so.SasaranOpd {
				if so.SasaranOpd[i].Id == int(sasaranId.Int64) {
					existingSasaran = &so.SasaranOpd[i]
					break
				}
			}
			if existingSasaran == nil {
				...
				existingSasaran = &so.SasaranOpd[len(so.SasaranOpd)-1]
			}
			...
			if indikatorId.Valid {
				...
				for i := range existingSasaran.Indikator {
					if existingSasaran.Indikator[i].Id == indikatorId.String {
						existingInd = &existingSasaran.Indikator[i]
						break
					}
				}
				if existingInd == nil {
					existingSasaran.Indikator = append(existingSasaran.Indikator, domain.Indikator{
						Id:               indikatorId.String,
						Indikator:        indikator.String,
						RumusPerhitungan: rumusPerhitungan,
						SumberData:       sumberData,
					...
					existingInd = &existingSasaran.Indikator[len(existingSasaran.Indikator)-1]
				}
				if targetId.Valid && targetTahun.Valid {
					existingInd.Target = append(existingInd.Target, domain.Target{
						Id:          targetId.String,
						IndikatorId: indikatorId.String,
						Tahun:       targetTahun.String,
						Target:      targetValue.String,
...
func (repository *SasaranOpdRepositoryImpl) FindByIdOnly(ctx context.Context, tx *sql.Tx, id int) (*domain.SasaranOpd, error) {
	script := `
    SELECT DISTINCT
        pk.id as pokin_id,
        pk.nama_pohon,
	...
	for rows.Next() {
		...
		err := rows.Scan(
			...
			&namaPegawai,
			&sasaranId, &namaSasaranOpd,
			&tahunAwalSasaran, &tahunAkhirSasaran,
			&jenisPeriodeSasaran,
			&idTujuanOpd,
		)
		if err != nil {
...
func (repository *SasaranOpdRepositoryImpl) FindIndikatorTargetsBySasaranIds(
	ctx context.Context,
	tx *sql.Tx,
	sasaranIds []int,
) ([]domain.Indikator, error) {
	...
	// buat placeholder (?, ?, ?, ...)
	placeholders := make([]string, len(sasaranIds))
	args := make([]any, len(sasaranIds))
	...
	query := fmt.Sprintf(`
		SELECT ind.id, ind.indikator, ind.sasaran_opd_id,
...
func (repository *SasaranOpdRepositoryImpl) FindIndikatorTargetsRenstraBySasaranIds(
	ctx context.Context,
	tx *sql.Tx,
	sasaranIds []int,
) ([]domain.Indikator, error) {
	...
	query := fmt.Sprintf(`
		SELECT ind.kode_indikator, ind.indikator, ind.sasaran_opd_id,
                     ind.rumus_perhitungan, ind.sumber_data,
...
func (repository *SasaranOpdRepositoryImpl) FindAllOnly(ctx context.Context, tx *sql.Tx, KodeOpd string, tahunAwal string, tahunAkhir string, jenisPeriode string) ([]domain.SasaranOpd, error) {
	script := `
    SELECT DISTINCT
        pk.id as pokin_id,
        pk.nama_pohon,
